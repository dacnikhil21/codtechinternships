...
export default function AdminDashboard() {
  ...
  const impersonateUser = async (id) => {
    const loadingToast = toast.loading('Initiating secure session...');
    try {
      ...
      if (data.success) {
        toast.success('Entering Admin View Mode');
        router.push('/dashboard');
      } else {
                    ...
                    {users.map((u) => (
                      <tr key={u.id} className="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td className="px-8 py-6 flex items-center gap-2">
                          <button onClick={() => impersonateUser(u.id)} title="View Student Dashboard" className="w-9 h-9 rounded-xl bg-indigo-500/10 text-indigo-500 hover:bg-indigo-500 hover:text-white transition-all flex items-center justify-center"><span className="material-symbols-outlined text-lg">visibility</span></button>
                          <button onClick={() => deleteUser(u.id)} title="Delete Student" className="w-9 h-9 rounded-xl bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white transition-all flex items-center justify-center"><span className="material-symbols-outlined text-lg">delete</span></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
