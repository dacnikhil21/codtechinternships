...
export default function AdminSidebar({ currentTab, onTabChange, isOpen, onClose }) {
  ...
  const menuItems = [
    ...
    { name: 'Support Requests', icon: 'support_agent', id: 'support' },
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full w-64 bg-slate-900 border-r border-white/5 flex flex-col z-[70] transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-8 mb-4 flex items-center justify-between">
          ...
          {/* Mobile Close Button */}
          <button onClick={onClose} className="lg:hidden w-8 h-8 flex items-center justify-center text-slate-500 hover:text-white">
