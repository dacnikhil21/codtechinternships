...
export default function Dashboard() {
  ...
  const handleSupportSubmit = async (e) => {
    ...
    if (user?.impersonatedBy) {
      return toast.error('Action disabled in Admin View Mode', { icon: '🚫' });
    }
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFF] flex w-full font-inter overflow-x-hidden text-slate-900 relative">
      ...
      <main className={`flex-1 transition-all lg:ml-64 p-4 md:p-8 lg:p-10 pt-24 lg:pt-10 min-h-screen max-w-full overflow-x-hidden`}>
        ...
        {user?.impersonatedBy && (
          <div className="bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-2xl mb-8 flex items-center justify-between shadow-sm">
             <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-red-100 text-red-600 rounded-xl flex items-center justify-center shrink-0">
                  <span className="material-symbols-outlined text-xl">admin_panel_settings</span>
                </div>
                <div>
                   <p className="text-sm font-black uppercase tracking-tight">Admin View Mode</p>
                   <p className="text-[10px] font-bold text-red-600/80 uppercase tracking-widest">Viewing Student Dashboard as {user.adminName || 'Admin'}</p>
                </div>
             </div>
             <button 
               onClick={async () => {
                 const toastId = toast.loading('Exiting student view...');
                 try {
               ...
               className="bg-red-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md shadow-red-500/20 hover:bg-red-700 transition-all flex items-center gap-2"
             >
                <span className="material-symbols-outlined text-sm">logout</span> Exit Student View
             </button>
          </div>
        )}
            ...
            <section className="bg-white p-6 rounded-3xl border border-slate-200/60 shadow-sm space-y-6">
               <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
                  ...
                  <div className="flex flex-col xl:flex-row gap-2 w-full lg:w-auto">
                    <button onClick={() => { setTempSelectedProjects(selectedProjects); setIsProjectModalOpen(true); }} className="flex-1 bg-indigo-600 text-white px-6 py-3 rounded-lg font-black text-[9px] uppercase tracking-[0.2em] hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 flex items-center justify-center gap-2 whitespace-nowrap">
                       <span className="material-symbols-outlined text-sm">add_circle</span> Browse Projects
                    </button>
                    <button onClick={() => setSelectedTask({ title: 'Project Implementation Guide' })} className="flex-1 bg-slate-50 text-slate-700 border border-slate-200/60 px-6 py-3 rounded-lg font-black text-[9px] uppercase tracking-[0.2em] hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 transition-all flex items-center justify-center gap-2 whitespace-nowrap">
                       <span className="material-symbols-outlined text-sm">auto_awesome</span> How to do project
                    </button>
                    <a href="https://drive.google.com/file/d/1T-3gvxOU06dSOiSZwm6CAlr7O9tj_22A/view?usp=drivesdk" target="_blank" rel="noopener noreferrer" className="flex-1 bg-slate-50 text-slate-700 border border-slate-200/60 px-6 py-3 rounded-lg font-black text-[9px] uppercase tracking-[0.2em] hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 transition-all flex items-center justify-center gap-2 whitespace-nowrap">
                       <span className="material-symbols-outlined text-sm">play_circle</span> Watch Project Guide
                    </a>
                  </div>
               </div>
